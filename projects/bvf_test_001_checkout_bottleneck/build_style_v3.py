"""
BVF Test 001 — Phase 4C: Bbox-driven camera, orange-only shutter, products, cross-frame preflight
"""
...
REVIEW_DIR = os.path.join(SCRIPT_DIR, "reviews", "style_v3")
...
def styled_shutter(name, x, y, z, is_middle=False):
    ...
    obj=bpy.context.object; obj.name=name; obj.scale=(0.9,0.03,0.7)
    color = ST["shutter_orange"] if is_middle else (0.25,0.23,0.20)
    obj.data.materials.append(mat_new(f"{name}_mat", color, 0.4))
    return obj
...
preflight_path=os.path.join(REVIEW_DIR,"composition_preflight_v3.json")
...
if not preflight["all_pass"]:
    ...
    for frame, label in [(1,"F001 Normal"),(90,"F090 Closed"),(150,"F150 Diversion"),(345,"F345 Congestion")]:
        scene.frame_set(frame)
        out=os.path.join(REVIEW_DIR,f"F{frame:03d}_style_v3.png")
        scene.render.filepath=out
        bpy.ops.render.render(write_still=True)
        print(f"  {label}: {out}")

    blend_path=os.path.join(SCRIPT_DIR,"scene_style_v3.blend")
