"""
BVF Test 001 — Phase 4A: Low Poly Miniature Visual Look Development
...
REVIEW_DIR = os.path.join(SCRIPT_DIR, "reviews", "style_v1")
...
ST = {
    ...
    "shutter": (0.91, 0.47, 0.19), "shelf": (0.28, 0.25, 0.22),
...
# ── Geometry Builders ──────────────────────────────────────
def lowpoly_character(root, name, skin_col, cloth_col):
    """Low-poly humanoid: head(ico), torso(cube), legs(2×cyl), arms(2×cyl)"""
    # Head: ico sphere
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2, radius=0.13, location=(0, 0, 0.98))
    head = bpy.context.object; head.name = f"{name}_Head"
    head.parent = root; head.data.materials.append(mat_new(f"{name}_skin", skin_col, 0.7))
    ...
    torso.parent = root; torso.scale = (0.13, 0.09, 0.18)
    torso.data.materials.append(mat_new(f"{name}_cloth", cloth_col, 0.8))

    # Legs L+R
    for side, sx in [("L", -0.04), ("R", 0.04)]:
        bpy.ops.mesh.primitive_cylinder_add(radius=0.035, depth=0.38, location=(sx, 0, 0.28))
        leg = bpy.context.object; leg.name = f"{name}_Leg{side}"
        leg.parent = root; leg.data.materials.append(mat_new(f"{name}_leg", (0.25,0.23,0.20), 0.9))

    # Arms L+R
    for side, sx in [("L", -0.16), ("R", 0.16)]:
        bpy.ops.mesh.primitive_cylinder_add(radius=0.028, depth=0.3, location=(sx, 0, 0.63))
        arm = bpy.context.object; arm.name = f"{name}_Arm{side}"
        arm.parent = root; arm.rotation_euler = (0.3, 0, 0.15* (1 if side=="R" else -1))
        arm.data.materials.append(mat_new(f"{name}_arm", skin_col, 0.7))

def build_lowpoly_char(name, x, y, cloth_idx):
    ...
    lowpoly_character(root, name, skin, cloth)
    return root
...
def build_cashier(name, x, y, z_off=0.0):
    ...
    lowpoly_character(root, f"Cashier_{name}", ST["char_skin"], ST["cashier_top"])
    return root

def styled_counter(name, x, y, z):
    """Counter with bevel, conveyor belt, scanner."""
    # Main body
    bpy.ops.mesh.primitive_cube_add(size=1, location=(x, y, z + 1.05/2))
    ctr = bpy.context.object; ctr.name = name
    ctr.scale = (1.0, 0.45, 1.05)
    ctr.data.materials.append(mat_new(f"{name}_mat", ST["counter"], 0.65))
    # Bevel modifier
    bm = ctr.modifiers.new("Bevel", 'BEVEL'); bm.width = 0.02; bm.segments = 2

    # Conveyor belt (flat strip on top, slightly recessed)
    bpy.ops.mesh.primitive_cube_add(size=1, location=(x, y-0.02, z + 1.05 + 0.02))
    belt = bpy.context.object; belt.name = f"{name}_Belt"
    belt.scale = (0.75, 0.38, 0.03)
    belt.data.materials.append(mat_new(f"{name}_belt", ST["conveyor"], 0.5))

    # Scanner bump
    bpy.ops.mesh.primitive_cube_add(size=1, location=(x, y+0.1, z + 1.05 + 0.06))
    scan = bpy.context.object; scan.name = f"{name}_Scanner"
    scan.scale = (0.12, 0.08, 0.06)
    scan.data.materials.append(mat_new(f"{name}_scan", ST["scanner"], 0.4))
...
# Counter overlay
bpy.ops.mesh.primitive_cube_add(size=1, location=(pm[0], sp["counter_y"], pm[2]+1.05/2))
overlay = bpy.context.object; overlay.name = "Counter_middle_overlay"
overlay.scale = (1.02, 0.47, 1.07)
om = mat_new("Overlay_mat", (0.08, 0.07, 0.06), 0.9, 0, 0.55)
...
for qkey in ["left_queue","middle_queue","right_queue"]:
    for i, cust in enumerate(CFG["characters_initial"][qkey]):
        ...
        root = build_lowpoly_char(cust["id"], sc[0], sc[1], ci)
...
for nc in CFG["new_customers"]:
    ...
    root = build_lowpoly_char(nc["id"], spx, spy, ci)
...
# ── Save ───────────────────────────────────────────────────
blend_path = os.path.join(SCRIPT_DIR, "scene_style_v1.blend")
bpy.ops.wm.save_mainfile(filepath=blend_path)
print(f"Saved: {blend_path}")
print("STYLE V1 COMPLETE")
