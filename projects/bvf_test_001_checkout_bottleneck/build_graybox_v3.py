"""
BVF Test 001 — V3: Character Hierarchy Fix + Camera Composition
...
REVIEW_DIR = os.path.join(SCRIPT_DIR, "reviews", "v3_character_fixed")
...
scene.world = bpy.data.worlds.new("GrayWorldV3")
...
# ── V3 Camera: Ortho, optimized for space utilization ─────
cam_data = bpy.data.cameras.new("Camera_V3")
cam_data.type = 'ORTHO'
cam_obj = bpy.data.objects.new("Camera_V3", cam_data)
...
print(f"Camera V3: ORTHO, pos={cam_obj.location}, ortho_scale={cam_data.ortho_scale}, shift_y={cam_data.shift_y}")
...
if not overall:
    ...
    print("\n=== All preflights passed. Rendering 6 stills... ===")
    for frame in RENDER_FRAMES:
        scene.frame_set(frame)
        out = os.path.join(REVIEW_DIR, f"F{frame:03d}.png")
        scene.render.filepath = out
        bpy.ops.render.render(write_still=True)
        print(f"  Frame {frame}: {out}")

# ── Save ───────────────────────────────────────────────────
blend_path = os.path.join(SCRIPT_DIR, "scene_graybox_v3_character_fixed.blend")
