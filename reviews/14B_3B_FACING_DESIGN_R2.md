# 14B-3B Facing Forward Axis -- Final Design R2
...
Final design incorporating R2A (configuration semantics), R2B (evidence + matrix strategy), R2B1 (direct evidence + overflow), R2C (scope guard contract + task split).
...
Only triggers when both axes are valid but tolerance is missing/null (the one gap 14A schema cannot cover). Error added to `input_errors`, exit code 2, `.blend` not opened. Final output sorted lexicographically by 14A `canonicalize()`.
...
Step 4: Validate and normalize
  ...
  4b. Length computation:
      try: length = math.sqrt(world_fwd[0]**2 + world_fwd[1]**2 + world_fwd[2]**2)
...
### 2.1 Overflow semantics (corrected from R2B1)

Finite component values (e.g. ~1e155) that pass 4a may produce `inf` during `x*x` multiplication in 4b. This `inf` is **not** caught by 4a (components were finite when checked). The `inf` propagates into `math.sqrt(inf) = inf` which passes without exception, then 4c catches it via `not isfinite(inf)`. Alternatively, the platform may raise `OverflowError` directly from `math.sqrt`, caught by the 4b except block. All paths route to `NORMALIZE_WORLD_FORWARD_AXIS, note=NONFINITE_WORLD_FORWARD_VECTOR`.
...
| x*x overflow | inf from x*x | -- | -- | ERROR NONFINITE |
...
Per-target order: descendants -> standing -> facing.
...
| All other functions | 0 | (not enforced) |

**`asset_scene_preflight_check.py`**:
- `.matrix_world` Load: 0 (unchanged)

### 9.3 Implementation notes for scope guard update

- Add a `_count_attr_calls(tree, attr_name)` helper similar to the existing `_count_matrix_world_attr_loads`
- For `.to_3x3()`: count `ast.Call` nodes whose `func` is an `ast.Attribute` with `attr == "to_3x3"`
- The verification must iterate all top-level functions, check the two authorized ones against their expected counts, and assert all others have count 0
...
```
14B_3B_I1A: _validate_facing_forward_axis_rules_preopen (pre-open validator)
14B_3B_I1B: _check_facing_forward_axis (NOT_CHECKED/PASS/FAIL + overall aggregation + integration in _check_root_objects)
```
...
NaN component
+Inf / -Inf component
x*x overflow (inf from finite components)
Negative scale, non-uniform scale, shear boundaries
```

### I3: Blender real mathutils
...
Standing+Facing joint execution verification. Scope guard test updated. Runner-production algorithm consistency test.
